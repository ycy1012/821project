...
def extract_and_save(features_type: str, image_paths: list, output_path: str):
    ...
    df = pd.DataFrame(results, columns=["filename"] + [f"feat_{i}" for i in range(len(results[0]) - 1)])
...
def main():
    ...
    parser.add_argument("--features", choices=["handcrafted", "deep", "all"], default="handcrafted",
                        help="Feature type to extract")

    args = parser.parse_args()

    image_paths = [os.path.join(args.input, f) for f in sorted(os.listdir(args.input))
                   if f.lower().endswith((".png", ".jpg", ".jpeg"))]

    if args.features in ["handcrafted", "all"]:
        extract_and_save("handcrafted", image_paths, os.path.join(args.output, "features_handcrafted.csv"))
    if args.features in ["deep", "all"]:
        extract_and_save("deep", image_paths, os.path.join(args.output, "features_deep.csv"))
