...
def validate_csv(csv_path, expected_files, expected_feat_dim):
    ...
    assert df.shape[1] == expected_feat_dim + 1, f"{csv_path}: expected {expected_feat_dim} features + filename."
    assert list(df["filename"]) == expected_files, f"{csv_path}: filenames do not match."
...
def test_run_pipeline_handcrafted_and_deep():
    ...
    run_pipline.extract_and_save("handcrafted", [os.path.join(sample_input_dir, f) for f in sample_files],
                                 os.path.join(output_dir, "features_handcrafted.csv"))
    run_pipline.extract_and_save("deep", [os.path.join(sample_input_dir, f) for f in sample_files],
                                 os.path.join(output_dir, "features_deep.csv"))

    # Validate handcrafted
    validate_csv(os.path.join(output_dir, "features_handcrafted.csv"), sample_files, expected_feat_dim=7)

    # Validate deep
    validate_csv(os.path.join(output_dir, "features_deep.csv"), sample_files, expected_feat_dim=512)
